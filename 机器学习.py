...
# # 项目简介
# 
# 目标检测算法主要分为两类，one-stage和two-stage检测方法，两段式检测方法采用bounding box进行边框回归，边框遍历从而检测到目标，通常准确率较高但实时性较差；而一段式检测方法直接进行分类和回归，一次性判断目标所属类别同时确定位置框，节省时间，故实时性较好，考虑到实时性对无人驾驶来说至关重要，故针对交通灯识别问题，我们选择YOLO系列神经网络进行训练模型。其运行速度快，使用图像全局信息做预测故误识别率较低，同时学习到的特征更加通用，推广适应性好。YOLO v3作为YOLO的官方最高版本，其基于YOLO和YOLO v2调整了网络结构；利用多尺度特征进行对象检测；在对象分类方面使用logistic取代了softmax，从而大幅提高了网络的实时性和准确率。

# # 交通灯数据集介绍
# 
# 在本项目中，将使用BOSCH开源的交通灯数据集。
# 这是一个基于视觉用于的交通灯检测的精确数据集。基于视觉的交通信号灯检测和跟踪是城市环境中实现全自动驾驶的关键一步。我们希望该数据集能够方便地测试目标检测方法，特别是对于较大图像中的小对象。
# 这些场景涵盖了相当多的道路场景和典型困难：
# * 繁忙的街景内景
# * 交通密度不同的郊区多车道道路
# * 密集的走走停停的交通
# * 道路工程
...
# 
# Bosch小型交通信号灯数据集，数据集大小大约为6GB，数据集申请地址：https://hci.iwr.uni-heidelberg.de/node/6132
# 数据集总共有5093张图片。将5093张图片手动划分为训练集、验证集和测试集，其中训练集2832张，验证集1684，测试集577张，分别存放在train、var、test文件目录下，训练集和验证集图片对应的xml文件也存放于与图片相同的文件目录下。每个xml文件是对一张图片的说明，包括图片尺寸、包含的交通灯类型、在图片上出现的位置等信息。示例如下：
# ![](https://ai-studio-static-online.cdn.bcebos.com/236d8c9ea4944e26bd6becd8eacddc1e0c4a396d8ea34e949d1f4d76b541b86b)
# 
# 
# ## 交通数据集结构如下：
# 
# * 总共5093张图片，其中训练集2832张，验证集1684，测试集577张。
# * 包含8种交通灯类型，分别是RedLeft、Red、RedRight、GreenLeft、Green、GreenRight、Yellow和off。
# * 包含了图片和标注，请读者先将数据解压，并存放在traffic_light目录下。

# In[7]:


# 解压数据脚本，第一次运行时打开注释，将文件解压到work目录下
get_ipython().system('unzip -q -d  /home/aistudio/work /home/aistudio/data/data119115/traffic_light.zip')
...
# # 结果分析和总结
# 
# 训练50个批次后，通过选取多张不同照片进行检测，交通灯基本上都能识别准确，面对可能与交通信号灯混淆的图像部件（例如大型圆形尾灯）也不会进行误判，可以看出构建的YOLOv3模型检测效果是足够好的。
# 
# 由于时间所限，只训练50个批次，如果想要更好的训练结果可以通过提高训练批次。当然数据集照片质量的好坏对训练后的检测结果影响也很大，由于使用的交通灯数据集照片质量较差，且涵盖了相当多复杂的道路场景，这都是训练模型时所要克服的挑战，但模拟真实的交通场景是必要的，因为现实中所面临的场景更为复杂，所要克服的困难也更多，在实际使用中，一个好的目标检测模型是非常有必要的，这也是做这个项目的动力所在。
